...
public class PlayerPrefsEditor : EditorWindow
{
	...
	private void DrawMainList()
    {
        ...
        for (int i = firstShownIndex; i < shownIndexLimit; i++)
        {
            ...
            if (valueType == typeof(float))
            {
                ...
                if (Math.Abs(newValue - initialValue) > 0f)
                {
	...
	private void DrawAddEntry()
	{
        ...
        newEntryType = (PlayerPrefType)GUILayout.SelectionGrid((int)newEntryType, new[] { "float", "int", "string" }, 3);
        EditorGUILayout.EndHorizontal();
    ...
    private void OnGUI()
    {
		...
		if(Application.platform == RuntimePlatform.OSXEditor)
		{
			...
			{
				GUILayout.Label("PList Last Written: " + lastWriteTime.ToString(CultureInfo.InvariantCulture));
			}
			else
			{
