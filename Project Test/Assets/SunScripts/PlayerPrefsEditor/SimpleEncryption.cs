...
namespace Sabresaurus.PlayerPrefsExtensions
{
	public static class SimpleEncryption
	{
		...
		private static void SetupProvider()
		{
			...
			provider = new RijndaelManaged {Key = Encoding.ASCII.GetBytes(key), Mode = CipherMode.ECB};
		...
		public static string EncryptString(string sourceString)
		{
			...
			// that string
			return Convert.ToBase64String(outputBytes ?? Array.Empty<byte>());
		}
		...
		public static string DecryptString(string sourceString)
		{
			...
			return Encoding.UTF8.GetString(outputBytes ?? Array.Empty<byte>());
		}
